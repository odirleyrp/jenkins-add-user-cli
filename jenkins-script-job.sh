...
if [ -e "tmp/" ]
then
echo " o diretorio tmp/  existe, apagando os arquivos dentro do diretorio"
rm -rf tmp/*
else
...
echo $GRUPO_JENKINS > tmp/jobs.txt
...
# Informar os usuários e JOBS coletado nas variáveis

for  PROJ in  $(cat $USER1); do
		echo " usuario definido é  $PROJ"
done

for  PROJ1 in  $(cat $JOBS1); do
		echo " O JOB  definido é  $PROJ1"
done
